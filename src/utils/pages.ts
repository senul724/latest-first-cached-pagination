...
export const calculatePageCount = (records: number) => {
  let pages = Math.floor(records / LIMIT_PER_PAGE);
...
export const getTotalPages = async () => {
  try {
    const pages = await redis.get("pages") as number;

    return { pages, err: false };
  } catch {
...
export const pageLimitAndOffset = async (
  ...
  try {
    ...
    const records = await redis.get("records") as number;
